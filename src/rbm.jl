...
begin
	...
	Grad{T} = Tuple{Matrix{T}, Vector{T}, Vector{T}}
	Samples{T} = NTuple{4, Matrix{T}}
...
begin
	...
	function sample!(::Type{Bernoulli}, x::Matrix{T}) where T
		r = rand(size(x)...)
		@. x = T(r < x)
	end
	
	function sample!(::Type{Unitary}, x::Matrix{T}) where T
		x
	end
	
	function nonlinearity!(::Type{Bernoulli}, x::Matrix{T}) where T
		@. x = σ(x)
	end
	
	nonlinearity!(::Type{Unitary}, x::Matrix{T}) where T = nonlinearity!(Bernoulli, x)
	
	function affine(W, bias::Vector{T}, x::Matrix{T}) where T
		W * x .+ bias
	end
		
	function sample_hiddens(rbm::AbstractRBM{T,V,H}, v::Matrix{T}) where {T,V,H}
	...
	function sample_visibles(rbm::AbstractRBM{T,V,H}, h::Matrix{T}) where {T,V,H}
...
begin
	...
	function batchify(X::Matrix{T}, batch_size::Int; shuffled=true) where T
		n = size(X, 2)
	...
	function CDk(rbm::AbstractRBM{T}, X::Matrix{T}, k::Int) where T
	...
	function dW(
		vpos::Matrix{T}, hpos::Matrix{T},
		vneg::Matrix{T}, hneg::Matrix{T}) where T
	...
	function db(vpos::Matrix{T}, vneg::Matrix{T}) where {T}
		Δb = dropdims(mean(vpos - vneg, dims=2), dims=2)
		
		Δb
	end
	
	function dc(hpos::Matrix{T}, hneg::Matrix{T}) where {T}
	...
	function pseudolikelihood(
		rbm::AbstractRBM{T,V,H}, batch::Matrix{T}; 
		n_vars::Int=1
	) where {T,V,H}
...
# ╔═╡ f545c7a6-7c60-11eb-3cce-8f71e26be5fd
rbm2 = BernoulliRBM(100, 20)
